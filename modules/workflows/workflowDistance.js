...
const workflowDistance = {
  process: ( req ) => {
    return new Promise( (resolve, reject) => {
      fhirQuestionnaire.processQuestionnaire( req.body ).then( (bundle) => {
        let practitioner = req.query.practitioner
        let distanceIndex = bundle.entry && bundle.entry.findIndex(entry => entry.resource.resourceType === 'Basic' && entry.resource.meta && entry.resource.meta.profile.includes('http://ihris.org/fhir/StructureDefinition/ihris-basic-distance'))
        let distance = bundle.entry[distanceIndex]
        distance.resource.extension.push({
...
module.exports = workflowDistance
