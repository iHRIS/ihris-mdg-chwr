...
const processLocation = async (dest, locations) => {
  try {
    ...
    for (let entry of bundle.entry) {
      ...
      locations.push(locDel)
      if (locations.length >= 100) {
        await saveBundle(locations)
        locations.length = 0 // Clear the locations array
      }
    }
    ...
    if (next && next.url) {
      ...
      await saveBundle(locations) // Save remaining locations if any
    }
